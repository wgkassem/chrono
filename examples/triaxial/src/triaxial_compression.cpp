...
int main(int argc, char* argv[]) {
    ...
    // randomize by layer
    ChVector<float> center(0.0f, 0.0f, -0.5f * sample_hgt + 5.1f * params.sphere_radius);
    // fill up each layer
    // particles start from 0 to cylinder_height/2
    while (center.z() + params.sphere_radius < 0.5f * sample_hgt - 5.1f * params.sphere_radius )  {
        auto points = sampler.SampleCylinderZ(center, sample_radius + 5.1f * params.sphere_radius, 0);
        initialPos.insert(initialPos.end(), points.begin(), points.end());
        center.z() += 5.1f * params.sphere_radius;
    }
